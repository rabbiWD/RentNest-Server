...
const createCheckoutSession = async (tenantId, payload) => {
    ...
    if (rentalRequest.status !== RentalRequestStatus.APPROVED) {
        throw new Error(`Cannot create payment. Rental request status is ${rentalRequest.status}. It must be APPROVED.`);
    }
...
const confirmPayment = async (tenantId, payload) => {
    if (!payload) {
...
const getMyPaymentHistory = async (tenantId) => {
    ...
    switch (event.type) {
        case "checkout.session.completed": {
            ...
            if (session.payment_status === "paid") {
                ...
                await completePaymentInDb(rentalRequestId, session.id, session.payment_method_types?.[0]);
            }
            break;
        }
        case "payment_intent.succeeded": {
            ...
            await completePaymentInDb(rentalRequestId, paymentIntent.id, paymentIntent.payment_method_types?.[0]);
            break;
        }
